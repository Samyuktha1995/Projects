...
def Run():
    ...
    #trainingCountry = populationTrainingMatrix[0, :]
    otherCountries = populationTrainingMatrix[1:,:]
    otherCountriesLabels = trainingCountries.iloc[1:]
    result, coefficientResult = iterationCountries(populationTrainingMatrix, trainingCountries)
    return result, coefficientResult
...
with open("coefficients.csv", "w", newline='') as f:
    writer = csv.writer(f)
    writer.writerows(selectedCoefficients)
